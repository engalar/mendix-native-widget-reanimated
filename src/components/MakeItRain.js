...
const ErlichBachman = ({ children }) => (
    <ImageBackground source={{ uri: erlich }} style={{ flex: 1 }}>
        {children}
    </ImageBackground>
);
            ...
            .map((flipDelay, i) => (
                ...
                >
                    <Swinging amplitude={MONEY_DIMENSIONS.width / 5} delay={randomize(duration)}>
                        <FlippingImage style={{ ...MONEY_DIMENSIONS }} source={{ uri: moneyFront }} delay={flipDelay} />
                        <FlippingImage
                            source={{ uri: moneyBack }}
                            delay={flipDelay}
                            back
                            style={{ position: "absolute", ...MONEY_DIMENSIONS }}
                        />
                    </Swinging>
                </Falling>
            ))}
